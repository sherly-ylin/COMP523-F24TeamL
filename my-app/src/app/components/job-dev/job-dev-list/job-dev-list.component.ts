...
export class JobDevListComponent implements OnInit {
  ...
  retrieveLogs(): void {
    this.JobDevService.getAll().subscribe({
      next: (data) => {
  ...
  deleteAll(): void {
    this.JobDevService.deleteAll().subscribe({
      next: (res) => {
        this.refreshList();
        console.log(res);
      },
  ...
  deleteOne(id: any): void {
    this.JobDevService.delete(id).subscribe({
      next: (res) => {
        this.refreshList();
        console.log(res);
      },
  ...
  editEntry(entry: JobDev) {
    this.JobDevService.update(entry.uid, entry).subscribe({
      next: (res) => {
        this.refreshList();
        console.log(res);
      },
